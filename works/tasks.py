...
def perform_screen(work_name, user_target, center_x, center_y, center_z, size_x, size_y, size_z, mol_db, pdb_path, res_path):
    ...
    email = UserProfile.objects.filter(id=VirtualScreen.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['email']
...
def perform_screen2(work_name, mol_db, pdb_path, resi_path, res_path):
    ...
    email = UserProfile.objects.filter(id=VirtualScreen2.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['email']
...
def perform_screen_user(work_name, center_x, center_y, center_z, size_x, size_y, size_z, user_db_name, pdb_path, res_path):
    ...
    screen_status(work_name, status='computing')
    email = UserProfile.objects.filter(id=VirtualScreen.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['email']
    username = UserProfile.objects.filter(id=VirtualScreen.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['username']
    screen_out = 'screen_out.csv'
    ...
    if screen_res:
        ...
        df.to_csv(screen_out, index=False)
        os.system("mv %s %s" % (screen_out, res))
        email_status(email, res_path + '/res/screen_out.csv',username,work_name)
    else:
        email_status(email,'',username,work_name)
...
def perform_screen2_user(work_name, user_db_name, pdb_path, resi_path, res_path):
    ...
    screen_status2(work_name, status='computing')
    email = UserProfile.objects.filter(id=VirtualScreen2.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['email']
    username = UserProfile.objects.filter(id=VirtualScreen2.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['username']
...
def perform_dock(work_name, center_x, center_y, center_z, size_x, size_y, size_z, pdb_path, lig_path, res_path):
    ...
    email = UserProfile.objects.filter(id=AutoDock.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['email']
    username = UserProfile.objects.filter(id=AutoDock.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['username']
    # 创建任务保存路径
    res = os.path.join(res_path, 'res')
    if not os.path.exists(res):
        os.mkdir(res)
    ...
    if os.path.exists(outqt_path):
        ...
        email_status(email, '/home/jianping/pywork/try27/drug/media/' + out_path.encode('raw_unicode_escape')+'qt',username,work_name)
...
# 定义用户指定关键残基分子对接任务
@shared_task
def perform_dock2(work_name, pdb_path, lig_path, resn, res_path):
    """
    :param work_name: 任务id
    ...
    """
    # 任务开始 修改任务状态为computing
    email = UserProfile.objects.filter(id=AutoDock2.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['email']
    username = UserProfile.objects.filter(id=AutoDock2.objects.filter(pdb_file=pdb_path.replace('/home/jianping/pywork/try27/drug/media/', '')).values()[0]['user_id']).values()[0]['username']
    dock_status2(work_name=work_name, status='computing')
    res = '%s/res' % res_path
    # 创建任务保存路径
    if not os.path.exists(res):
        os.mkdir(res)
    ...
    with open(resn.encode('raw_unicode_escape'), 'r') as f:
        lines2 = f.readlines()
    ...
    if os.path.exists(outqt_path):
        ...
        if out_lst:
            ...
            for model in out_lst:
                med.append(float(model.split()[0]))
            affinity = min(med)
            dock_affinity2(work_name=work_name, affinity=affinity)
        os.system("mv %s %s" % (outqt_path, res))
        os.system("python %s/extra_apps/vina/pdbqt_to_pdb.py -f %s -v" % (BASE_DIR, os.path.join(res, outqt)))
        out_path = os.path.join(res.split('media/')[1], outqt[:-2])
        dock_out2(work_name=work_name, out_path=out_path)
        email_status(email, '/home/jianping/pywork/try27/drug/media/' + out_path.encode('raw_unicode_escape')+'qt',username,work_name)
    # 分子对接任务完成,把任务状态修改为completed
    dock_status2(work_name=work_name, status='completed')
    # 分子渡劫任务完成,给用户发送邮箱
    # shutil.rmtree(res_path)
